...
module Azure
	module BaseManagement
		class ManagementHttpRequest < HttpRequest
			attr_accessor :uri, :warn, :key, :cert, :subscr_id
			...
			def initialize(method, path, body = nil, certificate_key, private_key, subscription_id)
				...
				@uri = URI.parse(Azure.config.management_endpoint + subscription_id + path)
				@key = private_key
			...
			def check_completion(request_id)
				request_path = "/#{self.subscr_id}/operations/#{request_id}"
				http = http_setup
