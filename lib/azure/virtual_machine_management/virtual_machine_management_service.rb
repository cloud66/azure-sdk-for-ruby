		...
		class VirtualMachineManagementService < BaseManagementService
			...
			def list_virtual_machines(*cloud_service_names)
				...
				if cloud_service_names.empty?
					cloud_service = Azure::CloudServiceManagementService.new(self.mng_cert, self.subscr_id)
			...
			def create_virtual_machine(params, options = {})
				...
				if options[:virtual_network_name]
					virtual_network_service = Azure::VirtualNetworkManagementService.new(self.mng_cert, self.subscr_id)
				...
				Azure::StorageManagementService.new(self.mng_cert, self.subscr_id).create_storage_account(options[:storage_account_name], optionals)
			...
			def add_role(params, options = {})
				...
				options[:storage_account_name] ||= generate_storage_account_name(params[:vm_name])
				Azure::StorageManagementService.new(self.mng_cert, self.subscr_id).create_storage_account(options[:storage_account_name], others)
			...
			def delete_virtual_machine(vm_name, cloud_service_name)
				...
				if vm
					...
					disk_management_service = VirtualMachineDiskManagementService.new(self.mng_cert, self.subscr_id)
			...
			def update_endpoints(vm_name, cloud_service_name, *input_endpoints)
				...
				if vm
					...
					request = ManagementHttpRequest.new(:put, path, body, self.cert_key, self.pr_key, self.subscr_id)
			...
			def get_os_type(image_name)
				image_service = Azure::VirtualMachineImageManagementService.new(self.mng_cert, self.subscr_id)
			...
			def validate_role_size(vm_size)
				valid_role_sizes = %w(extrasmall small medium large extralarge a5 a6 a7 a8 a9 a10 a11 standard_d1 standard_d2 standard_d3 standard_d4 standard_d11 standard_d12 standard_d13 standard_d14 d1_v2 d2_v2 d3_v2 d4_v2 d5_v2 d11_v2 d12_v2 d13_v2 d14_v2 f1 f2 f4 f8 f16 g1 g2 g3 g4 g5)
				if vm_size && !valid_role_sizes.include?(vm_size.downcase)
					Loggerx.error_with_exit "Value '#{vm_size}' specified for parameter 'vm_size' is invalid. Allowed values are 'ExtraSmall,Small,Medium,Large,ExtraLarge,A6,A7,A8,A9,A10,A11,Standard_D1,Standard_D2,Standard_D3,Standard_D4,Standard_D11,Standard_D12,Standard_D13,Standard_D14,D1_v2,D2_v2,D3_v2,D4_v2,D5_v2,D11_v2,D12_v2,D13_v2,D14_v2,F1,F2,F4,F8,F16,G1,G2,G3,G4,G5'"
				end
			end

			def validate_location(location_name)
				locations = Azure::BaseManagementService.new(self.mng_cert, self.subscr_id).list_locations
